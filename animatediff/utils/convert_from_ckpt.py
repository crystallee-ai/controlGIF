...
def conv_attn_to_linear(checkpoint):
    ...
    attn_keys = ["to_q.weight", "to_k.weight", "to_v.weight","to_out.0.weight"]
    for key in keys:
        if ".".join(key.split(".")[-2:]) in attn_keys:
            if checkpoint[key].ndim > 2:
                checkpoint[key] = checkpoint[key][:, :, 0, 0]
        elif ".".join(key.split(".")[-3:]) in attn_keys:
            if checkpoint[key].ndim > 2:
                checkpoint[key] = checkpoint[key][:, :, 0, 0]
...
def convert_ldm_clip_checkpoint(text_model, checkpoint):
    # text_model = CLIPTextModel.from_pretrained("/root/lh/clip-vit-large-patch14")
    names = []
