...
class AnimationPipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, video_length, height, width, dtype, device, generator, latents=None):
        ...
        if latents is None:
            ...
            # noise[:,:,0,:,:] =  (1. - init_weight) *  noise[:,:,0,:,:] + latents[:,:,0,:,:] * init_weight
            latents = noise
    ...
    def __call__(
        ...
        eta: float = 0.0,
        controlnet_image: torch.FloatTensor = None,
        ...
        **kwargs,
        ...
        image = prepare_image(
                ...
                width=controlnet_image.shape[-1],
                height=controlnet_image.shape[-2],
                batch_size=1,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                mid_block_res_sample = mid_block_res_sample.unsqueeze(2).repeat(1,1,video_length,1,1)
                for j in range(len(down_block_res_samples)):
                    down_block_res_samples[j] = down_block_res_samples[j].unsqueeze(2).repeat(1,1,video_length,1,1)
        ...
        # video = np.array(latents.cpu())
        # video = latents


        # warp-------
        # flows = torch.from_numpy(video)[0,:,:,:,:]
        # flows = flows.permute(1,2,3,0)
        # prev_image = controlnet_image.permute(1,2,0)

        # flows = np.array(flows) * 255.
        # prev_image = np.array(prev_image) * 255.
        # new_video = []
        # new_video.append(prev_image)
        # for i in range(flows.shape[0]):
        #     flow = flows[i, :, :, :2]
        #     vi = image_warp(prev_image, flow)
        #     new_video.append(vi)
        #     prev_image = vi
        # new_video = torch.tensor(new_video)
        # new_video = new_video.unsqueeze(0)
        # new_video = new_video.permute(0,4,1,2,3)
