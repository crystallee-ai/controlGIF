...
from clip_interrogator.clip_interrogator import Config, Interrogator, list_caption_models, list_clip_models
...
class AnimateController:
    def __init__(self):
        ...
        # self.stable_diffusion_dir   = os.path.join(self.basedir, "checkpoints", "stable-diffusion-v1-5")
        self.stable_diffusion_dir   = os.path.join(self.basedir, "checkpoints")
        self.motion_module_dir      = os.path.join(self.basedir, "checkpoints", "unet_temporal")
        self.personalized_model_dir = os.path.join(self.basedir, "checkpoints", "base_models")
        self.controlnet_model_dir   = os.path.join(self.basedir, "checkpoints", "controlnet")
    ...
    def refresh_controlnet_model(self):
        controlnet_model_list = glob(os.path.join(self.controlnet_model_dir, "*.ckpt"))
        self.controlnet_model_list = [os.path.basename(p) for p in controlnet_model_list]
    ...
    def update_stable_diffusion(self, stable_diffusion_dropdown):
        ...
        print("update stable diffusion done!")
    ...
    def update_motion_module(self, motion_module_dropdown):
        if self.unet is None:
            ...
            missing, unexpected = self.unet.load_state_dict(motion_module_state_dict["state_dict"], strict=False)
    ...
    def update_base_model(self, base_model_dropdown):
        if self.unet is None:
            ...
            self.text_encoder = convert_ldm_clip_checkpoint(self.text_encoder, base_model_state_dict)
    ...
    def update_controlnet_model(self, controlnet_dropdown):
        controlnet_dropdown = os.path.join(self.controlnet_model_dir, controlnet_dropdown)
        controlnet_state_dict = torch.load(controlnet_dropdown, map_location="cpu")
        missing, unexpected = self.controlnet.load_state_dict(controlnet_state_dict["state_dict"], strict=False)
        assert len(unexpected) == 0
        print("update controlnet done")
        return gr.Dropdown.update()
    
    def process_image(self, init_image, W, H):
        ...
        pixel_values = np.array(init_image)
    ...
    def animate(
        ...
        control_image = self.process_image(control_image, width_slider, height_slider)
...
def image_to_prompt(image):
    ...
    return [ci.interrogate(image), ci.interrogate_negative(image)]
        

controller = AnimateController()
config = Config(cache_path="./clip_interrogator/cache")
...
def ui():
    with gr.Blocks(css=css) as demo:
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                controlnet_model_dropdown = gr.Dropdown(
                    label="Select controlnet model (required)",
                    choices=controller.controlnet_model_list,
                    interactive=True,
                )
                controlnet_model_dropdown.change(fn=controller.update_controlnet_model, inputs=[controlnet_model_dropdown], outputs=[controlnet_model_dropdown])
        ...
        with gr.Column(variant="panel"):
            ...
            with gr.Column():
                with gr.Row():
                    control_image = gr.Image(type='pil', label="Image")
                    with gr.Column():
                        prompt_textbox = gr.Textbox(label="Prompt")
                        negative_prompt_textbox = gr.Textbox(label="Negative prompt")
            button = gr.Button(value="1. Generate prompt")
            button.click(fn=image_to_prompt, inputs=[control_image], outputs=[prompt_textbox, negative_prompt_textbox])
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    generate_button = gr.Button(value="2. Generate Animation", variant='primary')
